...
public:
	...
	unsigned char init(
		...
		float camera_d = 5.5f, /*distance camera is above drive plane in cm*/
		float camera_y = 10.f,/*distance end of camera is from center of robot in forward 'y' direction */
		float camera_ang=0.52f,/*angle the camera is deflected downwards*/
	...
	bool DriveplaneToScreenCameraCent(const s_2pt& XY, s_2pt& screenXY);
	...
	inline float getScreenHeight() { return m_y_pixH; }
	inline float getScreenWidth() { return m_x_pixW; }
protected:
	...
	/*reverse */
	bool Driveplane_Unit_d_To_screenFNorm(const s_2pt& XY, s_2pt& screenXY);/*converts robot coord norm by camera distance
																			 to screen pix coord norm by f_pix 
																			 focal length inpix */
	bool Driveplane_Unit_d_To_screen(const s_2pt& XY, s_2pt& screenXY);
};
